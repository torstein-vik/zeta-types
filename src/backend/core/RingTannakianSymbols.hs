...
trace :: (CMult m, CAdd m, CZModule m) => TS m -> m
trace symbol = foldr (+) zero $ fmap (uncurry (*#)) $ unSymbol symbol

pointCounts :: (Eq m, CMult m, CAdd m, CZModule m) => TS m -> [m]
pointCounts m = [e] ++ do
                k <- [1..]
...
bellAntiDerivative lst = memo where
                        ...
                        bd bdP 0 = e
                        bd bdP 1 = lst !! 1
                        bd bdP i = fromJust $ ((lst !! i) + foldr (+) zero (fmap (\j -> bdP (i - j) * (lst !! j)) [1..i - 1])) /# (toInteger i)